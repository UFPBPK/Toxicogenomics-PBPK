...
AUC <- readRDS(file = "AUC simulation.RDS") #This dataset is in the folder of Table 2/Results
...
BBMD <- read.csv("BBMD result mice.csv",header = TRUE) #In the folder of Figure 2/Results
BBMD.sig <- BBMD %>% filter(BMDU.BMDL < 40 & bmd < 10)

KEGG.m <- readRDS(file = "KEGG mice sig.RDS") #In the folder of Figure 3/Results
...
saveRDS(HED.pathway.mice, file = "Mouse HED for pathways.RDS") #The results for Figure 4
...
saveRDS(RfD, file = "Mouse RfD for significant pathways.RDS") #The results for Figure 5
