...
plot.kegg <- plot_grid(dot.D1+rremove("legend"), dot.D4+rremove("legend"), dot.D10+rremove("legend"), dot.D14+rremove("legend"),
                       nrow = 2, ncol = 2, align = c("hv"),rel_widths = c(5,5),
                       labels = c("A.","B.","C.","D."), label_fontfamily = "serif" )

plot.legend <- plot_grid(legend,nrow = 1,ncol = 1)
ggsave("Figure S3.tiff",scale = 1,
       plot = plot.kegg,
       width = 40, height = 30, units = "cm",dpi=320)
