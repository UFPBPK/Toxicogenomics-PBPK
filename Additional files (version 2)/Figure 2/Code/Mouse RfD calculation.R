...
AUC <- readRDS(file = "AUC simulation.RDS") #This dataset is in the folder of Table 1/Results

AUC.idv  = as.data.frame(AUC) # make a copy
## Estimated the average serum concentraiton (ASC)/average liver concentration (ALC)
AUC.idv <- AUC.idv %>% transmute(Avg.CA.mouse   = AUC.CA.mouse/(7*24), # mouse ASC: AUC normalized by exposure duration (7 days or 7*24 hours) 
                                 Avg.CA.human.m = AUC.CA.human.m/(365*50*24), # Human ASC: AUC normalized by the duration reaching the steady state (Based on the simulation results, it reached steady state at ~25 years.)   
                                 Avg.CL.mouse   = AUC.CL.mouse/(7*24),
                                 Avg.CL.human.m = AUC.CL.human.m/(365*50*24))
...
BBMD <- read.csv("BBMD result mice.csv",header = TRUE) #In the folder of Figure S1/Results
BBMD.sig <- BBMD %>% filter(BMDU.BMDL < 40 & bmd < 10)

KEGG.m <- readRDS(file = "KEGG mice sig.RDS") #In the folder of Figure S2/Results
...
saveRDS(HED.pathway.mice, file = "Mouse HED for pathways.RDS") #The results for Figure 2
...
saveRDS(RfD, file = "Mouse RfD for significant pathways.RDS") #The results for Figure 3
